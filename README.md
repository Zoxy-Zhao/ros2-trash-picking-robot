...
**抓取成功率：88%（项目作者提供）。** 作者反馈运行中基本无碰撞。该项目反馈与下文的软件自动测试结果分别列示，统计口径见 [验证说明](docs/software-validation.md#项目效果反馈)。
...
六轴软件模型为特定球腕构型，夹爪不计入六运动关节。提供路径点可达性与关节插值；软件自动测试覆盖运动学、轨迹、协议与模拟执行，项目抓取效果见上文。UART 发送或 C 解析成功不代表舵机到位；模拟急停也不等同于固件硬件急停。

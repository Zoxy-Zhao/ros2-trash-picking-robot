...
| 关节插值与分类抓取路径 | `move_joints.py`、`grasp_plan.py`、`arm_control.py` | 轨迹端点、速度、加速度与路径点 IK 检查 |
...
| 标定平面 X/Y 定位 | `plane_geometry.py`、`perspective.py` | 标定矩阵校验与坐标映射 |
...
运行 `python -m showcase.demo --firmware-check`，将合成检测、共用规划模块、串口编码和实际 C 解析器贯通。报告与测试结果见 [软件验证记录](software-validation.md)。

...
项目作者提供的抓取成功率为 **88%**，并反馈运行中基本无碰撞。该数据作为作者报告的项目效果列示，不是下面自动测试或合成样例的输出。

仓库尚未附对应的逐次抓取记录、成功次数/总次数、测试对象与成功判据，因此本报告不据此推算样本量或泛化成功率。“基本无碰撞”为运行情况的定性反馈，不表示已实现碰撞检测算法或完成所有路径的无碰撞证明。
...
本页的软件自动测试不覆盖机械碰撞、舵机跟踪、真实 UART 到位反馈、物理急停、相机标定精度或实际抓取效果；作者提供的项目效果单独列于上文。路径点提升至相同高度并不保证关节插值的整条 TCP 路径保持恒高。模拟接收检查模型限位，不等价于 STM32 的舵机 PWM 零位/行程校准。

YOLO/TensorRT 未在本机演示中运行；性能工具需在实际 Jetson 与模型环境执行。软件测试没有生成 FPS、抓取成功率或自主换桶完整闭环成绩。

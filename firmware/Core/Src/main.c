...
int main(void)
{
  ...
  HAL_Delay(20);
	Motors_Init();
	extern double K2[3];
	Forward_Ctrl_Init(K2[0], K2[1], K2[2]);
	transmit_Init();
	PCA_Servo_Init();
	lift_Init();
	int ret = 0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  /* USER CODE BEGIN Callback 0 */
	...
	if (htim == GAP_TIM) {
		...
		if(nums_gap == 4){
			nums_gap = 0;
			extern osSemaphoreId_t mpuSemaphoresHandle;
			BaseType_t pxHigherPriorityTaskWoken1 = pdFALSE;
			if(mpuSemaphoresHandle != NULL){
				xSemaphoreGiveFromISR(mpuSemaphoresHandle,
						&pxHigherPriorityTaskWoken1);
				portYIELD_FROM_ISR(pxHigherPriorityTaskWoken1);
			}

			extern osSemaphoreId_t controlsSemaphoresHandle;
			BaseType_t pxHigherPriorityTaskWoken2 = pdFALSE;
			if(controlsSemaphoresHandle != NULL){
	//			printf("controls\n");
				xSemaphoreGiveFromISR(controlsSemaphoresHandle,

...
int half_turns_nums = 0;
float current_pitch = 0, current_roll = 0, current_yaw = 0;
float last_pitch = 0, last_roll = 0, last_yaw = 0;
float pitch = 0, roll = 0, yaw = 0, prve_yaw = 0;
void MPU_Read(){
	if(MPU6050_DMP_Get_Date(&current_pitch, &current_roll, &current_yaw) == -1) return;
	float data_pitch = current_pitch - last_pitch;
	float data_roll = current_roll - last_roll;
	float data_yaw = current_yaw - last_yaw;
	if(current_pitch < 0.015 && current_pitch > -0.015) data_pitch = 0;
	if(current_roll < 0.015 && current_roll > -0.015) data_roll = 0;
	if(data_yaw < 0.0170 && data_yaw > -0.0170) data_yaw = 0;
	pitch += data_pitch;
	roll += data_roll;
	yaw += data_yaw;
	...
	if(fabs(yaw - prve_yaw) > 300){
		if(prve_yaw < 0) half_turns_nums++;
		else half_turns_nums--;
	}
	prve_yaw = yaw;
//	printf("%f   %d\n",yaw, half_turns_nums);
//	printf("%f\n",yaw-half_turns_nums*360);
}


float Gyro_GetYaw(){
	return yaw-half_turns_nums*360;
}
